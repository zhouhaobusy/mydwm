...
void runAutostart(void) { system("cd ~/scripts; ./autostart.sh &"); }
...
void togglefloating(const Arg *arg) {
  ...
  if (selmon->sel->isfloating && !(arg && arg->ui)) {
    ...
    selmon->sel->y = selmon->wy + selmon->wh / 6, randomxy(selmon->sel);
...
void setgap(const Arg *arg) {
  selmon->gappih = selmon->gappiv =
      arg->i ? MAX(selmon->gappiv + arg->i, 0) : gappi;
  selmon->gappoh = selmon->gappov =
      arg->i ? MAX(selmon->gappov + arg->i, 0) : gappo;
  arrange(selmon);
}
...
void tile(Monitor *m) {
  ...
  for (i = 0, my = ty = m->gappoh, c = nexttiled(m->clients); c;
       c = nexttiled(c->next), i++)
    if (i < m->nmaster) {
      r = MIN(n, m->nmaster) - i;
      h = (m->wh - my - m->gappoh - m->gappih * (r - 1)) / r;
      resize(c, m->wx + m->gappov, m->wy + my, mw - 2 * c->bw - m->gappiv,
             h - 2 * c->bw, 0);
      my += HEIGHT(c) + m->gappih;
    } else {
      r = n - i;
      h = (m->wh - ty - m->gappoh - m->gappih * (r - 1)) / r;
      resize(c, m->wx + mw + m->gappov, m->wy + ty,
             m->ww - mw - 2 * c->bw - 2 * m->gappov, h - 2 * c->bw, 0);
...
void grid(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next)) {
    ...
    resize(c, cx + m->gappoh, cy + m->gappov, cw - 2 * c->bw, ch - 2 * c->bw,
           0);
    i++;
  }
}
